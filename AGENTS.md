...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, `cursor`, and `omp`, plus `agy` for crewmates, scouts, and the primary, and `muse`, `gemini`, and `rovo` for crewmates and scouts only; never dispatch on an unverified adapter.
