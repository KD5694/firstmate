...
repair_line() {
  ...
  case "$HARNESS" in
    ...
    cursor)
      ...
      ;;
    agy)
      printf '%s%s\n' "$prefix" 'repair missing watcher supervision with bin/fm-watch-arm.sh as its own agy run_command call with WaitMsBeforeAsync 10000, never shell &.'
      ;;
    *)
...
ordinary_wake_line() {
  case "$HARNESS" in
    ...
    cursor)
      ...
      ;;
    agy)
      printf '%s\n' '- Ordinary wake: re-arm exactly one bin/fm-watch-arm.sh agy background run_command as directed below.'
      ;;
    *)
