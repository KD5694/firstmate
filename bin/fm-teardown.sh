...
# agy's per-task busy hooks are a firstmate-owned workspace plugin directory
# (bin/fm-spawn.sh). Remove only that directory's own files, then any plugin
# directories left empty, so a project's own .agents content is never touched.
remove_agy_task_plugin() {  # <worktree>
  local plugin="$1/.agents/plugins/firstmate-task"
  rm -f "$plugin/hooks.json" "$plugin/plugin.json"
  rmdir "$plugin" "$1/.agents/plugins" "$1/.agents" 2>/dev/null || true
}
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    if [ "$child_kind" = secondmate ]; then
      ...
      if [ -n "$child_wt" ] && [ -d "$child_wt" ]; then
        ...
        remove_agy_task_plugin "$child_wt"
      fi
