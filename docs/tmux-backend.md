...
omp is anchored to the exact `omp` identity for the same reason, so `ompd` and `comp` remain ambiguous, and agy is anchored to the exact `agy` identity, so `agyle` remains ambiguous.
