...
| --- | --- | --- |
| Run | Executes `bin/fm-session-start.sh` through the native session-open adapter and gates its ordered digest into model context before the first turn. | Claude, `codex exec`, Pi / pi-signed, omp, Cursor, agy |
...
| agy | Run | `.agents/hooks.json` registers `SessionStart` with a 180s timeout, invoked relative to the hooks file's own directory, and `bin/fm-agy-hook.sh` passes `--source startup` to the run wrapper and returns the digest as one persistent `userMessage` step encoded as `session-start` operational input. | agy fires `SessionStart` when the first prompt of a conversation is submitted, before the first model call, and again after `/clear`; its payload has no `source` field, so a `/clear` re-runs the full digest, which is redundant and idempotent (verified 1.2.1). |
| agy resume and compaction | Uncovered | None. | agy does not fire `SessionStart` for `--continue` or `--conversation`, and Firstmate registers nothing for compaction, so a resumed or compacted agy primary keeps whatever context survived and runs `bin/fm-session-start.sh` itself when the digest is absent (verified 1.2.1). |
...
Pi and agy inject a message rather than hook stdout, so whatever they inject must carry operational provenance or the Ahoy skill would have to guess whether it was captain-authored.
agy removes the leading U+2063 from a user message a hook injects, so its digest reaches the model starting at `FIRSTMATE_OP:`, and the agy supervision protocol names that digest as operational input.
...
`tests/fm-agy-primary.test.sh` proves the agy adapter over real processes under a stand-in `agy` parent: one marked `session-start` message routed as `startup`, silence in a child worktree, the harness identity reaching the deep session-start children, and a registration that steps aside when the adapter is absent.
