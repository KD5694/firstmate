...
Every supported primary harness was reviewed except omp and agy, whose rows below rest on bundled material or CLI help rather than a live enumeration.
...
| agy | present, per CLI help | Not wired and unverified. `agy --help` (1.2.1) lists an `--agent` flag and an `agents` subcommand for selecting and listing agents; no Firstmate delegation seatbelt is wired, and no live tool enumeration is recorded. |
