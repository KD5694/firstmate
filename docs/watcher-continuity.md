...
Grok retains its tracked background-task notification protocol, and agy uses the same shape through its `run_command` background task.
...
Claude depends on the Stop `asyncRewake` rewake, Cursor depends on its awaited stop-hook park, Grok and agy retain native background-completion notifications, and Codex retains bounded foreground checkpoints.
