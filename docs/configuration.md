...
agy is verified for crewmate and scout launches and as the primary, and `fm-spawn.sh` refuses it for a secondmate; its launch pre-registers the task worktree in agy's own trust store, and [`docs/verification/agy.md`](verification/agy.md) owns the evidence.
...
Claude's Stop `asyncRewake` hook owns tokenless re-arm cycles, Cursor's stop hook parks on the watcher, Grok and agy use background-notify cycles, Codex uses bounded foreground checkpoints, Pi and pi-signed use the same two tracked primary extensions, omp uses its own two tracked `.omp/extensions/` files with a blocking `session_stop` turn-end hook, agy uses the tracked `.agents/hooks.json`, and OpenCode uses its TUI plugin.
