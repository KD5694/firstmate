...
| agy | Not wired | Not needed: agy's `run_command` runs each call in its own shell at that call's explicit `Cwd`, so a top-level `cd` never persists into the next call (verified 1.2.1: `cd /usr` and then `pwd` as two separate calls printed the second call's `Cwd`). |
