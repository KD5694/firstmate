...
agy's dialog can be answered with Enter, but the spawn pre-registers the path in agy's own store too, because an untrusted launch runs its first turn without the task's busy and turn-end hooks.
